...
Read-only list and loan detail. On desktop, table rows and calendar event cards open `LoanDetailModal`; on phone, navigation goes to `/loans/[id]`. Month-view **+N more** stacks loan detail on top of the day events overlay. Row/card ⋯ menu matches `/loans` (including **Contract Details** → `LoanContractDetailsModal`). List modal and Contract Details use the same instant-row + background refresh and in-memory list patch as `/loans`. Sign CTA when a borrower signature slot is open: `/loans/[id]/sign`.
