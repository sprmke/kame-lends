...
**Analytics layout:** When the user has at least one group (`SHOW_GROUPS_UI` and non-empty `groupsIndex`), **Your groups** (`DashboardGroupsCard`) sits in a two-column row beside **Top investors** (`CurrencyBarChart`). The card header has **View all groups** (to `/groups`) instead of a folder icon. Cashflow (when transactions UI is on) stays full width above that row. Groups no longer appear under **Needs attention**.
...
- Empty activity shows all four panels with empty states. If any panel has loans, empty panels are hidden (all breakpoints). Visible panels share equal width and fill the row (`DashboardActivityCards`).
...
- **Phone (`<lg`):** chrome is the orange brand bar (logo + app name left, actions right) + floating glass tab dock (Dashboard, Groups, Loans, More, then the profile photo for Settings). The account sheet lists every nav link (including dock shortcuts and Settings), plus account, price visibility, and Light/Dark. The sheet hugs its content instead of filling the screen. `PageHeader` title sits in content below the bar; description stays hidden. Content uses `pt-mobile-top` / `pb-mobile-tab` clearance (scoped to `<lg` only; desktop has no extra top offset). Section eyebrows hide; metric cards are borderless with native float shadow. Price visibility is in the account sheet, not on the dashboard body. Empty metrics stay `hidden` on phone.
