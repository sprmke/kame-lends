...
**Date range + summary cards** match `/loans`: `from`/`to` due-date filter in the header (defaults to current month), or `?range=all` for all-time. Summary row (Principal, Total, Interest earned/estimate, Completed completed/total) scoped to the date range. Principal dedupes reused capital across non-overlapping loan periods; all-completed ranges show capital used. See `docs/guides/routes/loans.md` for URL and filter semantics.
