...
Read-only list and detail. On desktop, table rows and calendar event cards open `LoanDetailModal`; on phone, navigation goes to `/loans/[id]`. Month-view **+N more** stacks loan detail on top of the day events overlay. Row/card ⋯ menu includes **Contract Details** → `LoanContractDetailsModal`. Sign via `/loans/[id]/sign` when their witness slot is unsigned.
