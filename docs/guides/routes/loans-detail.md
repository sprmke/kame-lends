...
- **Commission:** `LoanSummarySection` shows borrower **Commission** / **Commission Rate** for everyone (not in Avg. Rate, Total Interest, or Total Amount). Party users edit **their** slot below the summary: borrower (`LoanBorrowerProfitCard`, `PATCH /api/loans/[id]/profit`), investor (`LoanInvestorCommissionCard`, `PATCH /api/loans/[id]/investors/[loanInvestorId]/commission`), witness (`LoanWitnessesSection`, `PATCH /api/loans/[id]/witnesses/[witnessLoanId]`). Rate mode shows principal and live preview; fixed mode omits principal. Party list tabs show `LoanCommissionSummaryCards` via `computePartyCommissionStats`. **Add Commission** in the row ⋯ menu opens detail with that editor active (`?commission=1` on phone).
