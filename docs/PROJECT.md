...
- **Floating dock** (`MobileTabBar`) sits above the home indicator: glass pill, solid orange active tab, 18px icons, 10px labels. Up to four primary destinations from `src/lib/nav/app-nav.ts`, plus **More**.
- **More sheet** lists overflow destinations, account, price visibility, a Light/Dark theme control, and sign out (13px rows, 44px min height). It slides up from the bottom with a fade frost scrim (shared Sheet primitive).
