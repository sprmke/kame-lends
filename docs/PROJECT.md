...
- **More sheet** lists every nav link (same set as the desktop sidebar, including dock shortcuts and Settings), then account chrome: profile, price visibility, Light/Dark theme, and sign out (13px rows, 44px min height). Height hugs content (max `92dvh`). It slides up from the bottom with a fade frost scrim (shared Sheet primitive).
