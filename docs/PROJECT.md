...
**AI receipt scanning:** `POST /api/ai/receipt-extraction` (workspace admin only) reads an uploaded payment receipt image and extracts sender/receiver name & bank, amount, date, and reference number — see `src/lib/server/ai/receipt-extraction.ts`. Gemini vision is primary (round-robins `GEMINI_API_KEYS`, falling back to a single `GEMINI_API_KEY`); on `429`/`403`/`5xx` from every Gemini key it falls back to Groq's Llama-4-Scout vision model (`GROQ_API_KEY`). Never throws — an unreadable image or missing keys returns `{ success: false, error }` so the caller (`ReceiptUploadField.svelte`) falls back to manual entry. Used from `LoanForm.svelte` (investor funding transactions, create/edit) and `LoanQuickPaymentDialog.svelte` (investor payments, borrower received payments): prefills amount/date without overwriting fields the admin already typed, and for investor payments fuzzy-matches the extracted sender name against the loan's lenders to auto-select one. The receipt image and the AI's extracted fields are stored directly on the `loan_investors` / `received_payments` row (`receiptImageUrl` data URL + `receiptExtractedData` jsonb, same convention as `validIdUrl`/`eSignatureUrl`) rather than a separate table, so they survive the loan-edit form's full delete/reinsert of those rows.
