...
**Object storage:** `POST /api/storage/upload-url` (authenticated) returns a presigned R2 PUT URL and a `storage:` reference. `GET /api/storage/object?ref=storage:…` checks RBAC, then redirects to a short-lived presigned download URL for UI previews. Server modules: `src/lib/server/storage/`.
