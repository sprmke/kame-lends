...
**Party commission:** borrower (`loans.profitType`/`profitValue`), witness (`loan_witnesses`), and investor (`loan_investors.profitType`/`profitValue`, migration `0023_loan_investor_commission.sql`). Each party edits their own slot without being the loan manager: `PATCH /api/loans/[id]/profit` (owner or borrower), `PATCH /api/loans/[id]/witnesses/[witnessLoanId]` (owner or linked witness), `PATCH /api/loans/[id]/investors/[loanInvestorId]/commission` (owner or linked investor). List stats: `computePartyCommissionStats` on **Commissioned** only (`?scope=commissioned` = union of party loan IDs). UI: `LoanCommissionSummaryCards`, `LoanBorrowerProfitCard`, `LoanInvestorCommissionCard`, `LoanWitnessesSection`. Borrower commission in `LoanSummarySection` is separate from investor interest totals. Legacy name in code: `profit*` columns.
