...
- **2026-09-09:** old `pawn-tracker@...` SA returned `invalid_grant: account not found`. Do not borrow the kame-homes service account. Create a new SA in the kame-lends / pawn-tracker GCP project, share a non-`primary` QA calendar with that email, and keep secrets in `.env.local` only.
