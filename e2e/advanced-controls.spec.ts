...
test("settings page renders maintenance controls", async ({ page }) => {
  ...
  await expect(
    page.getByRole("button", { name: "Sync Due Dates" }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Fix Payments" }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Download Backup" }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Calendar sync" }),
  ).toBeVisible();
});
...
test("loan create form opens new borrower modal", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Create Loan" }),
  ).toBeVisible();
});

test("valid signing page renders signing controls", async ({
  page,
  request,
}) => {
  ...
  const dueDate = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)
    ...
    .slice(0, 10);
  const e2eEmail =
    process.env.E2E_USER_EMAIL?.trim() || `e2e-sign-inv-${stamp}@example.com`;
  ...
  const borrowerRes = await request.post("/api/borrowers", {
    data: {
      name: `E2E Sign Borrower ${stamp}`,
      email: `e2e-sign-bor-${stamp}@example.com`,
    },
  });
  ...
  const invitation = signingData.invitations.find((item) =>
    item.signingUrl.includes(`/loans/${loan.id}/sign`),
  );
  ...
  await expect(
    page.getByRole("button", { name: "I Agree and Sign Electronically" }),
  ).toBeVisible();
...
test("loan duplicate action pre-fills create form", async ({
  page,
  request,
}) => {
  const stamp = Date.now();
  ...
  expect(response?.status()).toBe(200);
  await expect(
    page.getByRole("heading", { name: `E2E Dup Loan ${stamp}` }),
  ).toBeVisible({
    timeout: 20_000,
  });
  await page.waitForLoadState("networkidle");
  ...
  await expect(
    page.getByRole("heading", { name: "Duplicate Loan" }),
  ).toBeVisible({
    timeout: 20_000,
  });
