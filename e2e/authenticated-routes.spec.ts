...
test('loans list renders with table columns', async ({ page }) => {
	...
	if (await emptyState.isVisible()) {
		await expect(page.getByRole('link', { name: /create your first loan/i })).toBeVisible();
		return;
	}
...
test('investor detail renders', async ({ page, request }) => {
	const investors = await fetchJson<Array<{ id: number }>>(request, '/api/investors');
...
test('borrowing create form renders', async ({ page }) => {
	...
	await expect(page.getByText('Select Investors', { exact: true })).toBeVisible();
});
...
test('borrower detail renders', async ({ page, request }) => {
	const borrowers = await fetchJson<Array<{ id: number }>>(request, '/api/borrowers?simple=true');
