...
import { fetchJson, gotoApp, skipIfNoDatabase, waitForLoansTable } from './helpers';
...
test('loan quick-view modal shows summary and actions', async ({ page }) => {
	...
	await expect(dialog.getByText('Total Principal', { exact: true })).toBeVisible();
	await expect(dialog.getByText('Total Interest', { exact: true })).toBeVisible();
...
test('loan create modal opens and closes', async ({ page }) => {
	...
	await expect(dialog.getByRole('heading', { name: 'Create Loan' })).toBeVisible({
		timeout: 20_000
	});
...
test('borrowing quick-view modal opens', async ({ page, request }) => {
	...
	expect(response?.status()).toBe(200);
	await expect(page.getByRole('heading', { name: 'Borrowings', exact: true })).toBeVisible();

	await page.locator('table tbody tr').first().click();
	const dialog = page.getByRole('dialog');
	await expect(dialog).toBeVisible({ timeout: 15_000 });
	await expect(dialog.getByText('Principal', { exact: true }).first()).toBeVisible();
