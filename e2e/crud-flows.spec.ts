...
test("create and delete an investor", async ({ page, request }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Create Investor" }),
  ).toBeVisible({
    timeout: 20_000,
  });
  ...
  await page
    ...
    .fill(`e2e-investor-${stamp}@example.com`);
  ...
  const createResponsePromise = page.waitForResponse((res) =>
    res.url().includes("/api/investors"),
  );
  ...
  const created = await fetchJson<Array<Investor>>(
    request,
    "/api/investors?simple=true",
  ).then((list) => list?.find((i) => i.email === email));
  expect(created).toBeDefined();

  await page
    .getByPlaceholder("Search investors...")
    .fill(`E2E Investor ${stamp}`);
  await expect(
...
test("edit an investor", async ({ page, request }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Edit Investor", exact: true }),
  ).toBeVisible();
...
test("create and delete a transaction", async ({ page, request }) => {
  ...
  const transactions = await fetchJson<Array<Transaction>>(
    request,
    "/api/transactions",
  );
  const created = transactions?.find(
    (t) => t.name === `E2E Transaction ${stamp}`,
  );
  expect(created).toBeDefined();
...
test("create and delete a borrowing", async ({ page, request }) => {
  ...
  const debts = await fetchJson<Array<{ id: number; name: string }>>(
    request,
    "/api/debts",
  );
...
test("create and delete a loan", async ({ page, request }) => {
  ...
  await page
    .locator('#investors-section input[type="number"]')
    .first()
    .fill("50000");

  const createResponsePromise = page.waitForResponse((res) =>
    res.url().includes("/api/loans"),
  );
  ...
  await page.waitForResponse((res) =>
    res.url().includes(`/api/loans/${loanId}/signing`),
  );
  await page.waitForLoadState("networkidle");
