 ...
 */
export function isMoreThanOneMonth(startDate: Date | string, endDate: Date | string): boolean {
...
export function generateDefaultInterestPeriods(
	...
	const start =
		typeof sentDate === 'string' ? fromLocalDateString(sentDate) : normalizeToMidnight(sentDate);
	const end =
		typeof dueDate === 'string' ? fromLocalDateString(dueDate) : normalizeToMidnight(dueDate);
