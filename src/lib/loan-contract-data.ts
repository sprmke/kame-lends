...
import { getCollateralSummary, getContractTitle } from '$lib/loan-contract-content';
...
export function normalizeLoanContractData(data: LoanContractData): LoanContractData {
	return {
		...data,
...
export function formatContractCurrency(amount: number | string | null | undefined): string {
...
function formatDraftInterestDescription(allocation: LoanContractDraftInvestor): string {
...
function getEarliestSentDateFromDraft(investors: LoanContractDraftInvestor[]): Date {
	const sentDates = investors
		.flatMap((allocation) => allocation.transactions.map((t) => new Date(t.sentDate)))
	...
	return sentDates.reduce((earliest, date) => (date < earliest ? date : earliest));
}
...
export function buildLoanContractDataFromDraft(draft: LoanContractDraftInput): LoanContractData {
	...
	const parsedFreeLotSqm = freeLotSqm !== null && !Number.isNaN(freeLotSqm) ? freeLotSqm : null;
	...
	return {
		...
		collateralDescription: getCollateralSummary(draft.type, draft.loanTitleLabel, parsedFreeLotSqm),
...
export function getContractDetailRows(data: LoanContractData): ContractDetailRow[] {
...
export function buildLoanContractData(loan: LoanWithInvestors): LoanContractData {
	...
	investorGroups.forEach((transactions, investorId) => {
		...
		lenders.push({
			...
			principalAmount: transactions.reduce((sum, t) => sum + (parseFloat(t.amount) || 0), 0),
	...
	return {
		...
		collateralDescription: getCollateralSummary(loan.type, loan.loanName, loan.freeLotSqm),
