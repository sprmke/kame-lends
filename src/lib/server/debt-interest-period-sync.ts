...
export async function recalculateDebtInterestPeriodStatus(periodId: number): Promise<void> {
	...
	const status = derivePeriodStatus(paidTotal, expectedAmount, new Date(period.dueDate));
...
export async function syncDebtInterestPeriods(debtId: number): Promise<void> {
	...
	const existingByNumber = new Map(existing.map((period) => [period.periodNumber, period]));
	...
	for (const [, period] of existingByNumber) {
		if ((period.receivedPayments ?? []).length === 0) {
			await db.delete(debtInterestPeriods).where(eq(debtInterestPeriods.id, period.id));
		}
	}
}

export async function markOverdueDebtInterestPeriods(debtId: number): Promise<void> {
...
export async function deleteDebtReceivedPaymentsForPeriod(periodId: number): Promise<void> {
	await db
