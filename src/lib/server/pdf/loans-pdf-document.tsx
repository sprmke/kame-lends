...
import type { LoanWithInvestors, LoanStatus, LoanType, InterestPeriodStatus } from '$lib/types';
...
function getStatusColor(status: LoanStatus): string {
	switch (status) {
		case 'Fully Funded':
			return PDF_COLORS.fullyFunded;
		case 'Partially Funded':
			return PDF_COLORS.partiallyFunded;
		case 'Completed':
			return PDF_COLORS.completed;
		case 'Overdue':
			return PDF_COLORS.overdue;
		default:
...
function getTypeColor(type: LoanType): string {
	switch (type) {
		case 'Lot Title':
			return PDF_COLORS.lotTitle;
		case 'OR/CR':
			return PDF_COLORS.orcr;
		case 'Agent':
			return PDF_COLORS.agent;
		default:
			return PDF_COLORS.secondary;
	}
}
...
const TypeBadge = ({ type }: { type: LoanType }) => (
	<View
		style={[
			styles.badge,
			{ backgroundColor: getTypeColor(type) + '22', border: `1 solid ${getTypeColor(type)}` }
		]}
	>
		...
		<View style={[styles.loanCard, isLast ? { marginBottom: 0 } : {}]} break={!isFirst}>
			...
			{visibleSectionKeys.length === 0 && (
				<View style={styles.sectionNoBorder}>
					<Text style={{ fontSize: 8, color: PDF_COLORS.muted }}>
						No additional sections enabled.
					</Text>
				</View>
			)}
			...
			{showFinancial &&
				(() => {
					const totalReceived = loan.loanInvestors.reduce(
						(sum, li) =>
							sum +
							(li.receivedPayments || []).reduce((t, rp) => t + (parseFloat(rp.amount) || 0), 0),
						0
					);
						...
						<View style={sectionStyle('financial_summary')} wrap={false}>
							...
							<View style={styles.financialGrid}>
								<View style={styles.financialCell}>
									...
									<Text style={styles.financialCellValue}>
										{formatCurrencyForPDF(stats.totalPrincipal)}
									</Text>
								</View>
								<View style={styles.financialCell}>
								...
								<View style={styles.financialCell}>
									...
									<Text style={styles.financialCellValue}>
										{formatCurrencyForPDF(stats.totalInterest)}
									</Text>
								</View>
								<View style={styles.financialCell}>
									<Text style={styles.financialCellLabel}>Final Due Date</Text>
									<Text style={styles.financialCellValueSmall}>
										{formatDateForPDF(loan.dueDate)}
									</Text>
								</View>
							</View>
							...
							<View style={[styles.financialGrid, { marginTop: 6 }]}>
								<View
									style={[
										...
										{ backgroundColor: PDF_COLORS.accentBlueBg, border: '1 solid #dbeafe' }
									]}
								>
							...
							>
								...
								<View wrap={false}>
									<View style={styles.investorHeader}>
										<Text style={styles.investorName}>{formatTextForPDF(li.investor.name)}</Text>
									</View>
									...
									<View style={styles.investorDetails}>
										<View style={styles.investorDetailItem}>
											...
											<Text style={styles.investorDetailValue}>
												{formatDateForPDF(li.sentDate)}
											</Text>
										</View>
										<View style={styles.investorDetailItem}>
											<Text style={styles.investorDetailLabel}>Principal</Text>
											<Text style={styles.investorDetailValue}>
												{formatCurrencyForPDF(li.amount)}
											</Text>
										</View>
										<View style={styles.investorDetailItem}>
									...
									<View style={{ paddingHorizontal: 10, paddingBottom: 8 }}>
										...
										<View style={styles.periodTable}>
											<View style={styles.periodTableHeader}>
												...
												<Text style={[styles.periodTableHeaderCell, { flex: 1.5 }]}>
													Rate / Amount
												</Text>
												...
												.map((period, pIdx) => {
													...
													return (
														<View
															key={period.id}
															style={
																isLastPeriod ? styles.periodTableRowLast : styles.periodTableRow
															}
															wrap={false}
														>
															<Text
																style={[
																	...
																	{ flex: 0.6, color: PDF_COLORS.muted }
																]}
															>
																{pIdx + 1}
															</Text>
											...
											{visiblePayments.map((payment, rIdx) => {
												...
												return (
													<View
														key={payment.id}
														style={
															isLastPayment ? styles.paymentsTableRowLast : styles.paymentsTableRow
														}
														wrap={false}
													>
														<Text style={[styles.periodTableCell, { flex: 2 }]}>
			...
			{showNotes && hasNotes && (
				<View style={sectionStyle('notes')}>
					<Text style={styles.sectionLabel} minPresenceAhead={20}>
						Notes
					</Text>
...
export const LoansPDFDocument = ({
	...
	const grandTotalReceived = data.reduce((sum, loan) => {
		return (
			sum +
			getRelevantInvestors(loan).reduce(
				(lSum, li) =>
					lSum + (li.receivedPayments || []).reduce((t, rp) => t + (parseFloat(rp.amount) || 0), 0),
				0
			)
		);
	}, 0);
	const grandTotalBalance = Math.max(0, grandTotalAmount - grandTotalReceived);
	const grandTotalLotSqm = data.reduce((sum, loan) => sum + (loan.freeLotSqm ?? 0), 0);
		...
		>
			<Page size="A4" style={styles.page}>
					...
					<View style={[styles.totalsCard, { marginTop: 0, marginBottom: 16 }]} wrap={false}>
						<View style={styles.totalsHeader}>
							<Text style={styles.totalsHeaderText}>
								Grand Totals — {data.length} Loan{data.length !== 1 ? 's' : ''}
							</Text>
						</View>
						...
						<View style={[styles.totalsGrid, { paddingBottom: 4 }]}>
							<View style={styles.totalsCell}>
								...
								<Text style={styles.totalsCellValue}>
									{formatCurrencyForPDF(grandTotalPrincipal)}
								</Text>
							</View>
							<View style={styles.totalsCell}>
								<Text style={styles.totalsCellLabel}>Total Interest</Text>
								<Text style={styles.totalsCellValue}>
									{formatCurrencyForPDF(grandTotalInterest)}
								</Text>
							</View>
							<View style={styles.totalsCell}>
							...
							<View style={styles.totalsCell}>
								...
								<Text
									style={[
										styles.totalsCellValue,
										{ color: grandTotalBalance <= 0 ? PDF_COLORS.fullyFunded : PDF_COLORS.lotTitle }
									]}
								>
						...
						<View
							style={[
								styles.totalsGrid,
								{ paddingTop: 0, paddingBottom: 12, borderTop: `0.5 solid #dbeafe` }
							]}
						>
				...
				<View style={styles.pageFooter} fixed>
					<Text style={styles.footerText}>{APP_NAME} — Confidential</Text>
					<Text
...
export async function renderLoansPDF(
	...
	const blob = await pdf(
		<LoansPDFDocument data={data} enabledSections={enabledSectionKeys} investorId={investorId} />
	).toBlob();
