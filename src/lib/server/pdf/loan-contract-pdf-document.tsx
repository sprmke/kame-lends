...
function SignatureBlock({ party }: { party: SignaturePartyDetails }) {
	return (
		<View style={styles.signatureBlock} wrap={false}>
			...
			<Text style={styles.signatureField}>Printed Name: {party.printedName}</Text>
...
function LoanContractPDFDocument({
	...
	const displayData = customization ? applyContractCustomization(data, customization) : data;
	const lenderNames =
	...
	const signatureParties = getContractSignaturePartiesForDisplay(displayData, customization);
		...
		<Document>
			<Page size="LETTER" style={styles.page} wrap>
				<View style={styles.header}>
					...
					<Text style={styles.subtitle}>Contract No. {displayData.contractNumber}</Text>
				</View>

				<View style={styles.section}>
					<Text style={styles.paragraph}>{getContractIntroText(displayData, customization)}</Text>
				</View>
				...
				{displayData.lenders.length > 0 && (
					<View style={styles.section}>
						<Text style={styles.sectionTitle} minPresenceAhead={MIN_LENDER_BLOCK}>
							Lender Allocation
						</Text>
						{displayData.lenders.map((lender) => (
							<View key={lender.email} style={styles.lenderBlock} wrap={false}>
								...
								{lender.contactNumber ? (
									<Text style={styles.lenderMeta}>Contact: {lender.contactNumber}</Text>
								) : null}
								...
								<Text style={styles.lenderMeta}>Interest: {lender.interestDescription}</Text>
							</View>
						))}
					</View>
				)}
				...
				<View style={styles.signatureSection}>
					<View style={styles.signatureSectionHeader} wrap={false}>
						<Text style={styles.signatureIntro} minPresenceAhead={MIN_SIGNATURE_BLOCK}>
							{getWitnessAttestationText(customization)}
						</Text>
					</View>
					<View style={styles.signatureGrid}>
						{signatureParties.map((party) => (
							<SignatureBlock key={`${party.role}-${party.printedName}`} party={party} />
						))}
					</View>
				</View>
...
export async function renderLoanContractPDF(
	...
	const blob = await pdf(
		<LoanContractPDFDocument data={contractData} customization={customization} />
	).toBlob();
