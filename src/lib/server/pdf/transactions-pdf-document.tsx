...
import type { TransactionWithInvestor, TransactionDirection } from '$lib/types';
...
export const TransactionsPDFDocument = ({
	data,
	enabledSections,
	...
	function getCellValue(tx: TransactionWithInvestor, key: string) {
		switch (key) {
			case 'date':
				return formatDateForPDF(tx.date);
			case 'name':
				return formatTextForPDF(tx.name);
			case 'investor':
				return formatTextForPDF(tx.investor.name);
			case 'type':
				return formatTextForPDF(tx.type);
			case 'amount':
				return formatCurrencyForPDF(tx.amount);
			case 'notes':
				return tx.notes || '—';
			default:
				return '—';
	...
	function getTotalsValue(key: string): string {
		switch (key) {
			case 'date':
				return 'TOTAL';
			case 'amount':
				return `In: ${formatCurrencyForPDF(totalIn)}  Out: ${formatCurrencyForPDF(totalOut)}`;
			default:
				return '';
		}
	}
	...
	return (
		<Document
			title={`Transaction Report — ${APP_NAME}`}
			author={APP_NAME}
			creator={APP_NAME}
							...
							return (
								<View key={tx.id} style={rowStyle}>
									{cols.map((col) => {
												...
												<Text
													...
													style={[
														styles.tableCellBold,
														{
															flex: col.flex,
															color:
																tx.direction === 'In' ? PDF_COLORS.fullyFunded : PDF_COLORS.overdue
														}
													]}
												>
													{tx.direction === 'In' ? '+' : '-'}
										...
										if (col.key === 'notes') {
											return (
												<Text key={col.key} style={[styles.tableCellNotes, { flex: col.flex }]}>
													{getCellValue(tx, col.key)}
												</Text>
											);
										}
