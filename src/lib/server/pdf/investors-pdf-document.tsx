...
import { calculateInvestorStats, calculateAverageRate } from '$lib/calculations';
...
export const InvestorsPDFDocument = ({
	data,
	enabledSections,
	...
	function getCellValue(investor: InvestorWithLoans, key: string): string {
		...
		switch (key) {
			case 'name':
				return formatTextForPDF(investor.name);
			case 'email':
				return formatTextForPDF(investor.email || '—');
			case 'phone':
				return formatTextForPDF(investor.contactNumber || '—');
			case 'capital':
				return formatCurrencyForPDF(stats.totalCapital);
			case 'rate':
				return formatRateForPDF(avgRate);
			case 'interest':
				return formatCurrencyForPDF(stats.totalInterest);
			case 'balance':
				return formatCurrencyForPDF(stats.currentBalance);
			case 'gain':
				return formatCurrencyForPDF(stats.totalGain);
			case 'active':
				return formatCountForPDF(stats.activeLoans);
			case 'completed':
				return formatCountForPDF(stats.completedLoans);
			case 'overdue':
				return formatCountForPDF(stats.overdueLoans);
			case 'total':
				return formatCountForPDF(stats.totalLoans);
			default:
				return '—';
		}
	}
	...
	function getTotalsValue(key: string): string {
		switch (key) {
			...
			case 'phone':
				return '';
			case 'capital':
				return formatCurrencyForPDF(totalCapital);
			case 'interest':
				return formatCurrencyForPDF(totalInterest);
			case 'balance':
				return formatCurrencyForPDF(totalBalance);
			case 'gain':
				return formatCurrencyForPDF(totalGain);
			case 'active':
				return String(totalActive);
			case 'completed':
	...
	return (
		<Document
			title={`Investor Report — ${APP_NAME}`}
			author={APP_NAME}
			creator={APP_NAME}
				...
				{data.length === 0 ? (
					...
					<View style={styles.table}>
						...
						<View style={styles.totalsRow}>
							{cols.map((col) => (
								<Text key={col.key} style={[styles.totalsCellValue, { flex: col.flex }]}>
