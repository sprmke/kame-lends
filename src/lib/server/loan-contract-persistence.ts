...
import { buildSigningInvitationsForLoan, getWitnessInvitationPartyName } from '$lib/loan-signing';
import type { SigningPartyRole } from '$lib/loan-signing';
import type { LoanWithInvestors } from '$lib/types';

function signingInvitationKey(partyRole: SigningPartyRole, investorId?: number | null): string {
	if (partyRole === 'lender') {
...
export async function saveLoanContractAndInvitations(
	...
	const invitations = await insertSigningInvitations(loan, contract.id, customization);
...
async function insertSigningInvitations(
	...
	const invitationInputs = buildSigningInvitationsForLoan(loan, contractId, customization);
	...
	for (const partyRole of witnessRoles) {
		...
		await db
			...
			.set({
				...
				partyEmail:
					(isFirst ? customization.witness1Email : customization.witness2Email)?.trim() || null,
				witnessId: isFirst ? customization.witness1Id : customization.witness2Id
			})
			.where(
				and(
...
export async function syncSigningInvitationsForLoan(loan: LoanWithInvestors) {
	...
	const existingKeys = new Set(
		existing.map((invitation) => signingInvitationKey(invitation.partyRole, invitation.investorId))
	);

	const expected = buildSigningInvitationsForLoan(loan, contract.id, customization);

	const missing = expected.filter(
		(input) => !existingKeys.has(signingInvitationKey(input.partyRole, input.investorId ?? null))
	);
