...
import { loans, investors, transactions, loanInvestors } from '$lib/server/db/schema';
...
export async function fetchBackupDataForUser(options: {
	...
	let allTransactions: Awaited<ReturnType<typeof db.query.transactions.findMany>>;
	...
	return {
		...
		excludedFromExport: ['user (auth)', 'account (OAuth tokens)', 'session', 'verificationToken'],
		data: {
