...
function createEventSummary(eventData: CalendarEventData): string {
	...
	if (type === 'summary') {
		...
		if (type === 'sent') {
			return `${loan.type}: ${loan.loanName} - Disbursement (-${formatCurrency(totalAmount || 0)})`;
		} else if (type === 'due') {
			return `${loan.type}: ${
				loan.loanName
			} - Due Date (${investorName} +${formatCurrency(totalAmount || 0)})`;
		} else if (type === 'interest_due') {
			return `${loan.type}: ${loan.loanName} - Interest Due (${investorName} +${formatCurrency(
...
function getEventColor(type: 'sent' | 'due' | 'interest_due' | 'summary'): string {
...
export async function deleteMultipleCalendarEvents(eventIds: string[]): Promise<void> {
...
export async function deleteAllCalendarEvents(): Promise<number> {
	try {
		if (
			...
			console.warn('Google Calendar credentials not configured. Skipping calendar cleanup.');
			return 0;
		}
...
export async function updateDailySummaryEvents(
	...
	try {
		if (
			...
			console.warn('Google Calendar credentials not configured. Skipping summary event update.');
			return;
		}
		...
		for (const loan of allLoans) {
			...
			const hasAnyMultipleInterest = loan.loanInvestors.some(
				(li) => li.hasMultipleInterest && li.interestPeriods && li.interestPeriods.length > 0
			);

			if (hasAnyMultipleInterest) {
				// Process interest periods
				for (const li of loan.loanInvestors) {
					if (li.hasMultipleInterest && li.interestPeriods && li.interestPeriods.length > 0) {
		...
		for (const [dateKey, dayData] of dailyEvents.entries()) {
			...
			// Remove duplicates
			const uniqueLoans = Array.from(new Map(allLoansForDay.map((l) => [l.id, l])).values());
			...
			const existingEventId = await findExistingEventByDateAndPrefix(date, 'Daily Summary');
...
export async function generateLoanCalendarEvents(loan: LoanWithInvestors): Promise<string[]> {
