...
import { buildLoanContractData, type LoanContractData } from './loan-contract-data';
...
export function buildAuthenticatedSigningUrl(loanId: number, origin?: string): string {
...
export function emailsMatch(provided: string, expected: string | null | undefined): boolean {
...
export function applySigningSignatures(
	...
	for (const invitation of invitations) {
		...
		switch (invitation.partyRole) {
			...
			case 'lender': {
				const lenderEmail =
					invitation.investorId != null ? investorEmailById.get(invitation.investorId) : undefined;
...
export function applyLiveSignaturePreview(
	...
	switch (partyRole) {
		...
		case 'lender':
			if (partyEmail) {
				nextData.lenders = nextData.lenders.map((lender) =>
					lender.email === partyEmail ? { ...lender, eSignatureUrl: signatureDataUrl } : lender
				);
			}
			break;
		case 'witness_1':
...
export function buildInvestorEmailMap(loan: LoanWithInvestors): Map<number, string> {
...
export function sortSigningInvitations<T extends { id: number; partyRole: SigningPartyRole }>(
	invitations: T[]
): T[] {
	return [...invitations].sort((a, b) => {
		const roleDiff = SIGNING_ROLE_ORDER[a.partyRole] - SIGNING_ROLE_ORDER[b.partyRole];
...
export function toSigningInvitationSummary(
	...
	return {
		...
		signedAt: invitation.signedAt ? new Date(invitation.signedAt).toISOString() : null,
