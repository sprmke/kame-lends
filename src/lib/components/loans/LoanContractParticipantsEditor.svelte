			...
			<Popover.Content class="w-[var(--bits-popover-anchor-width)] p-0">
				...
				<div class="max-h-64 overflow-y-auto p-1">
					{#if filteredContacts().length}
						...
						{/each}
					{:else}
						<div class="dashboard-empty px-3 text-sm text-muted-foreground">
...
<div class="space-y-5">
	...
	<div class="space-y-4 rounded-xl border border-border bg-muted/20 p-4">
		...
		{:else}
			<div
				class="dashboard-empty rounded-md border border-dashed border-border px-4 text-sm text-muted-foreground"
			>
