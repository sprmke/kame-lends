...
import { formatCurrency, formatDate, formatRateLabel, formatCount } from '$lib/format';
...
export function formatReceivedDatesCommaSeparated(rows: ReceivedPaymentDisplay[]): string {
...
export function matchReceivedDatesToCompletedPeriods(
	...
	for (const period of periodsSortedByDue) {
		if (period.status !== 'Completed' || typeof period.id !== 'number') continue;
		...
		for (let i = 0; i < pool.length; i++) {
			...
			if (Math.abs(pool[i].amount - expected) > AMOUNT_MATCH_TOLERANCE) continue;
...
export function computeLoanTotalPrincipal(investors: InvestorWithTransactions[]): number {
	return investors.reduce(
		(sum, item) => sum + item.transactions.reduce((s, t) => s + (parseFloat(t.amount) || 0), 0),
		0
	);
}
...
export function computeInvestorMetrics(
	...
	const totalCapital = transactions.reduce((sum, t) => sum + (parseFloat(t.amount) || 0), 0);

	const hasMultiplePeriods =
		!!item.hasMultipleInterest && !!item.interestPeriods && item.interestPeriods.length > 1;
	...
	const totalReceived = receivedPayments.reduce((sum, rp) => sum + (parseFloat(rp.amount) || 0), 0);
	...
	const periodPrincipalBase = totalCapital === 0 ? loanTotalPrincipal : totalCapital;
...
export function sectionKey(investorId: number, section: 'principal' | 'interest'): string {
