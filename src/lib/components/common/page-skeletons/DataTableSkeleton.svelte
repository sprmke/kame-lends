...
<Card.Root class={cn('overflow-hidden border-border/40', className)}>
	<Card.Content class="p-0">
		<div class="flex items-center gap-2 border-b border-border/50 bg-muted/30 px-3 py-2">
		...
		{#each Array.from({ length: rows }) as _, rowIndex (rowIndex)}
			<div
				class={cn(
					'flex items-center gap-2 border-b border-border/50 px-3 last:border-0',
					tallRows ? 'py-4' : 'py-3'
				)}
			>
				{#each columns as col, i (i)}
		...
		{/each}
		<div
			class="flex flex-col items-start justify-between gap-3 border-t border-border/50 px-3 py-3 sm:flex-row sm:items-center"
		>
