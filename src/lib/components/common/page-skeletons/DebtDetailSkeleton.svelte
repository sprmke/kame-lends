<script lang="ts">
	...
	import SkeletonMetricGrid from './SkeletonMetricGrid.svelte';
		...
		<Card.Content class="space-y-4">
			<div class="grid grid-cols-2 gap-3 md:grid-cols-3">
				{#each Array.from({ length: 9 }) as _, index (index)}
					<div class="rounded-2xl border border-border/50 bg-background p-3">
			...
			<div class="space-y-2">
				...
				{#each Array.from({ length: scheduleRows }) as _, index (index)}
					<div class="flex items-start justify-between gap-3 rounded-2xl border border-border/50 bg-background p-3">
						...
						<div class="space-y-1.5 text-right">
							<Skeleton class="ml-auto h-5 w-32" />
							<Skeleton class="ml-auto h-3 w-40" />
						</div>
					</div>
				{/each}
				<Skeleton class="mt-2 h-11 w-full rounded-2xl" />
			</div>
		</Card.Content>
	</Card.Root>
</div>
