...
export const USER_PERSISTED_CUSTOMIZATION_FIELDS: ReadonlyArray<keyof ContractCustomization> = [
	'witness1Id',
	'witness1Name',
	'witness1Email',
	'witness1Address',
...
function mergeCustomizationField<K extends keyof ContractCustomization>(
	...
	if (field === 'lenderDateSigned') {
		return (
			dirtyFields.has(field)
				? mergeLenderSignatureDates(previous.lenderDateSigned, defaults.lenderDateSigned)
				: defaults.lenderDateSigned
		) as ContractCustomization[K];
	}

	return (dirtyFields.has(field) ? previous[field] : defaults[field]) as ContractCustomization[K];
}
...
export function mergeCustomizationWithDefaults(
	...
	return {
		contractTitle: mergeCustomizationField('contractTitle', previous, defaults, dirtyFields),
		...
		securityClause: mergeCustomizationField('securityClause', previous, defaults, dirtyFields),
		disputeVenue: mergeCustomizationField('disputeVenue', previous, defaults, dirtyFields),
		witness1Id: mergeCustomizationField('witness1Id', previous, defaults, dirtyFields),
		witness1Name: mergeCustomizationField('witness1Name', previous, defaults, dirtyFields),
		witness1Email: mergeCustomizationField('witness1Email', previous, defaults, dirtyFields),
		witness1Address: mergeCustomizationField('witness1Address', previous, defaults, dirtyFields),
		...
		witness2Id: mergeCustomizationField('witness2Id', previous, defaults, dirtyFields),
		witness2Name: mergeCustomizationField('witness2Name', previous, defaults, dirtyFields),
		witness2Email: mergeCustomizationField('witness2Email', previous, defaults, dirtyFields),
		witness2Address: mergeCustomizationField('witness2Address', previous, defaults, dirtyFields),
		...
		includeWitnesses: mergeCustomizationField('includeWitnesses', previous, defaults, dirtyFields),
		...
		lenderDateSigned: mergeCustomizationField('lenderDateSigned', previous, defaults, dirtyFields),
		...
		additionalTerms: mergeCustomizationField('additionalTerms', previous, defaults, dirtyFields)
	};
}
...
export function isBorrowerSignatureIncluded(customization?: ContractCustomization): boolean {
...
export function areContractCustomizationsEqual(
	left: ContractCustomization,
	right: ContractCustomization
): boolean {
	const normalize = (customization: ContractCustomization) => ({
		...customization,
		lenderSignaturesIncluded: sortRecordKeys(customization.lenderSignaturesIncluded),
		lenderDateSigned: sortRecordKeys(customization.lenderDateSigned)
	});
