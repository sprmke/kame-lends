...
export function calculateDebtExpectedInterest(debt: DebtLike): number {
  return calculateDebtSummary(debtSummaryInput(debt))
...
type DebtWithTimeline = DebtLike & {
  interestPeriods?: DebtInterestPeriodWithPayments[];
  updatedAt?: Date;
};

function getDebtCapitalInterval(
  debt: DebtWithTimeline,
): CapitalInterval | null {
  const principal = calculateDebtPrincipal(debt);
  if (principal <= 0) return null;

  const startKey = toCapitalDayKey(debt.date);
  const endKey = isFullyPaidDebt(debt)
    ? toCapitalDayKey(
        debt.updatedAt ??
          getDebtEndDate(
            debt.date,
...
function computePeakConcurrentDebtPrincipal(
  debts: DebtWithTimeline[],
  from: string | null = null,
  to: string | null = null,
): number {
  const intervals = debts
    .map(getDebtCapitalInterval)
...
export function calculateInvestorDebtStats(
  ...
  const totalPrincipal = computePeakConcurrentDebtPrincipal(debts);
  const activePrincipal = computePeakConcurrentDebtPrincipal(activeDebts);
  const completedPrincipal = computePeakConcurrentDebtPrincipal(completedDebts);
  ...
  const totalRepayment = debts.reduce(
    (sum, debt) =>
      sum + calculateDebtSummary(debtSummaryInput(debt)).totalRepayment,
    0,
  );
