...
export function calculateAdditionalFeesTotal(fees: DebtAdditionalFee[] | null | undefined): number {
...
export function generateInterestSchedule(
	...
	const principalAmount = safeParseFloat(principal);
	const rate = periodRateFromPercent(interestRate);
	const installment = calculateAmortizedPayment(principalAmount, interestRate, periodCount);
...
export function calculateDebtSummary(input: {
	...
	const additionalFeesTotal = calculateAdditionalFeesTotal(normalizeDebtFees(input.additionalFees));
	const schedulePeriods =
		input.schedulePeriods ??
		durationMonthsToSchedulePeriods(input.durationMonths ?? 12, input.interestInterval);
	const schedule = generateInterestSchedule(
		input.debtDate,
		principal,
		interestRate,
	...
	const scheduleInterestTotal = schedule[schedule.length - 1]?.cumulativeInterest ?? 0;
	...
	return {
		...
		totalInterestIncludingFees: scheduleInterestTotal + additionalFeesTotal,
...
export function isCompletedDebt(
	...
	const end = getDebtEndDate(debt.date, debt.durationMonths ?? DEFAULT_DEBT_DURATION_MONTHS);
	return today >= end;
}
...
export function calculateDebtInterestCostPaid(
	...
	const scheduleByPeriod = new Map(summary.schedule.map((entry) => [entry.period, entry]));
	let cost = 0;
...
export function calculateDebtAccruedInterest(debt: DebtLike, asOf: Date = new Date()): number {
	const today = normalizeDebtDate(asOf);
...
export function calculateInvestorDebtStats(
	...
	const totalPrincipal = debts.reduce((sum, debt) => sum + calculateDebtPrincipal(debt), 0);
	const activePrincipal = activeDebts.reduce((sum, debt) => sum + calculateDebtPrincipal(debt), 0);
	...
	const interestPaid = debts.reduce((sum, debt) => sum + calculateDebtInterestCostPaid(debt), 0);
