...
export function buildLoanCalendarEvents(loans: LoanWithInvestors[]): CalendarEvent[] {
	...
	loans.forEach((loan) => {
		...
		if (hasAnyMultipleInterest) {
			...
			const totalPrincipal = loan.loanInvestors.reduce((sum, li) => sum + parseFloat(li.amount), 0);
