...
import { loans, loanInvestors, interestPeriods } from '$lib/server/db/schema';
...
export const POST: RequestHandler = async (event) => {
	...
	try {
		...
		const latestDueDate = new Date(sortedEntries[sortedEntries.length - 1].dueDate);
		...
		const investorGroups = new Map<number, (typeof loan.loanInvestors)[number][]>();
		...
		for (const [, transactions] of investorGroups) {
			const investorCapital = transactions.reduce((sum, t) => sum + safeParseFloat(t.amount), 0);
			...
			const transactionWithPeriods = transactions.find(
				(t) => t.hasMultipleInterest && t.interestPeriods && t.interestPeriods.length > 0
			);
			...
			const originalInvestorInterest = calculateInterestAmount(base, baseRate, baseType);
			...
			const newPeriods = sortedEntries.map((entry) => {
				const isEdited = Math.abs(entry.totalAmount - entry.originalAmount) > 0.01;
