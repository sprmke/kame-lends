...
import { hasLoanAdminAccess, hasInvestorAllocationAccess, hasLoanAccess } from '$lib/server/access-control';
...
export const POST: RequestHandler = async (event) => {
	...
	try {
		...
		const investorIdPreview = Number.parseInt(String(preview.investorId ?? ''), 10);
		const allowed =
			(await hasLoanAdminAccess(loanId, session.user.id)) ||
			(Number.isFinite(investorIdPreview) &&
				(await hasInvestorAllocationAccess(loanId, session.user.id, investorIdPreview)));
		if (!allowed) {
		...
		if (!Number.isFinite(investorId)) {
			return json({ error: 'Select the lender receiving this payment.' }, { status: 400 });
		}
		if (!Number.isFinite(amount) || amount <= 0) {
			return json({ error: 'Enter an amount greater than zero.' }, { status: 400 });
		}
		if (Number.isNaN(receivedDate.getTime())) {
			return json({ error: 'A valid received date is required.' }, { status: 400 });
		}
		...
		const lenderPayments = await db.query.loanInvestors.findMany({
			where: and(eq(loanInvestors.loanId, loanId), eq(loanInvestors.investorId, investorId)),
			with: {
		...
		if (!lenderPayments.length) {
			return json({ error: 'The selected lender is not assigned to this loan.' }, { status: 400 });
		}
		...
		if (requestedPeriodId !== null) {
			await recalculateInterestPeriodStatusFromLinkedPayments(requestedPeriodId);
