...
export const POST: RequestHandler = async (event) => {
	...
	try {
		...
		if (
			...
			!(await hasInvestorAllocationAccess(loanId, session.user.id, investorId))
		) {
		...
		if (!Number.isFinite(investorId)) {
			return json({ error: 'Select a lender for this payment.' }, { status: 400 });
		}
		...
		if (duplicateDate) {
			return json(
				{
					error: 'This lender already has a principal payment on the selected date.'
				},
				{ status: 400 }
			);
		}
		...
		const scheduleSource = existingPayments.find(
			(payment) => payment.hasMultipleInterest && payment.interestPeriods.length > 0
		);
