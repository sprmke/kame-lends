		...
		if (investorData && investorData.length > 0) {
			...
			for (let i = 0; i < investorData.length; i++) {
				...
				if (
					...
					console.log('Inserting interest periods for loanInvestorId:', loanInvestorId, periodData);
			...
			for (const entry of receivedPaymentsByInvestor) {
				const loanInvestorId = investorToLoanInvestorId.get(Number(entry.investorId));
				if (!loanInvestorId || !entry.receivedPayments || !Array.isArray(entry.receivedPayments)) {
					continue;
				}
...
export const DELETE: RequestHandler = async (event) => {
	...
	try {
		...
		await db.delete(loans).where(and(eq(loans.id, loanId), eq(loans.userId, session.user.id)));
