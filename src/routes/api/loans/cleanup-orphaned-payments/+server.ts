...
import { loans, loanInvestors, receivedPayments, interestPeriods } from '$lib/server/db/schema';
	...
	try {
		...
		for (const loan of userLoans) {
			...
			for (const li of loan.loanInvestors) {
				...
				const completedPeriods = li.interestPeriods.filter((p) => p.status === 'Completed');
				...
				const missingPeriods = completedPeriods.filter((p) => !linkedPeriodIds.has(p.id));
		...
		if (multiInterestLiIds.length > 0) {
			const orphans = await db.query.receivedPayments.findMany({
				where: (rp, { and, isNull: qIsNull, inArray: qInArray }) =>
					and(qIsNull(rp.interestPeriodId), qInArray(rp.loanInvestorId, multiInterestLiIds))
			});

			if (orphans.length > 0) {
				const orphanIds = orphans.map((rp) => rp.id);
				await db.delete(receivedPayments).where(inArray(receivedPayments.id, orphanIds));
				...
				for (const loan of userLoans) {
					if (loan.loanInvestors.some((li) => affectedLiIds.has(li.id))) {
		...
		return json({
			...
			message:
				parts.length > 0
					? parts.join(', ') +
						(affectedLoans.length > 0 ? ` — loans: ${affectedLoans.join(', ')}` : '')
