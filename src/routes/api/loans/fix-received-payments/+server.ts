...
import { loanInvestors, receivedPayments, loans } from '$lib/server/db/schema';
			...
			for (const li of loan.loanInvestors) {
				...
				for (const period of missingPeriods) {
					...
					const interestAmount =
						period.interestType === 'fixed' ? rate : principalBase * (rate / 100);
		...
		if (multiInterestLiIds.length > 0) {
			const orphans = await db.query.receivedPayments.findMany({
				where: (rp, { and: qAnd, isNull: qIsNull, inArray: qInArray }) =>
					qAnd(qIsNull(rp.interestPeriodId), qInArray(rp.loanInvestorId, multiInterestLiIds))
			});
