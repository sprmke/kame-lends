<script lang="ts">
	...
	const emptyMessage = $derived((data as { emptyMessage?: string }).emptyMessage ?? 'No loans yet');
	const listScope = $derived((data as { listScope?: string }).listScope ?? 'owned');
	...
	const rowActions = $derived(
		loanListRowActionHandlers({
			scope: listScope === 'owned' ? 'loans' : (listScope as 'investments' | 'borrowed' | 'witnessed'),
			canManage,
...
{:else}
	<DashboardPage>
		<PageHeader title={pageTitle} description="Manage all your loans" showPriceToggle={true}>
		...
		{:else if loans.length === 0}
			<ListEmptyState message={emptyMessage} icon={FileText} />
